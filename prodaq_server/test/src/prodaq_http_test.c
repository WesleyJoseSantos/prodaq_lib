...
void prodaq_http_request_get_url_test(void)
{
    ...
    prodaq_err_t err = prodaq_http_request_get_url(request, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_HEX(PRODAQ_OK, err);
    ...
    TEST_ASSERT_EQUAL_HEX(PRODAQ_ERR_HTTP_REQUEST_PARSE_FAIL, err);
    ...
    TEST_ASSERT_EQUAL_HEX(PRODAQ_ERR_BUFFER_OVERFLOW, err);
}
...
void prodaq_http_request_get_body_success_test(void)
{
    char request[] = "GET /index.html HTTP/1.1\r\n"
                     ...
                     "abcdefg";
    char buffer[8];
    prodaq_err_t result = prodaq_http_request_get_body(request, buffer, sizeof(buffer));
...
void prodaq_http_request_get_body_buffer_too_small_test(void)
{
    ...
    char buffer[6];
    prodaq_err_t result = prodaq_http_request_get_body(request, buffer, sizeof(buffer));
    TEST_ASSERT_EQUAL(PRODAQ_ERR_BUFFER_OVERFLOW, result);
}
...
void prodaq_http_request_get_body_parse_fail_test(void)
{
    ...
    char buffer[8];
    prodaq_err_t result = prodaq_http_request_get_body(request, buffer, sizeof(buffer));
    TEST_ASSERT_EQUAL(PRODAQ_ERR_HTTP_REQUEST_PARSE_FAIL, result);
}
...
void test_prodaq_http_parse_request_success(void)
{
    ...
    prodaq_err_t expected_err = PRODAQ_OK;

    // When
    prodaq_err_t actual_err = prodaq_http_parse_request(request_str, &request);
...
void test_prodaq_http_parse_request_url_parse_fail(void)
{
    ...
    http_request_t request;
    prodaq_err_t expected_err = PRODAQ_ERR_HTTP_REQUEST_PARSE_FAIL;
...
void test_prodaq_http_parse_request_url_buffer_too_small(void)
{
    ...
    http_request_t request;
    prodaq_err_t expected_err = PRODAQ_ERR_BUFFER_OVERFLOW;
